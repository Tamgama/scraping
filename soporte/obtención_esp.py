...
headers = {
    ...
    "cookie": cookie,
    "Referer": "https://www.idealista.com/alquiler-viviendas/jaen-provincia/",
...
zonas = ['La Campiña', 'COmarca norte - El Condado', 'Sierra de Segura', 'Jaén', 'La Loma y las VIllas', 'Alto Guadalquivir', 'Sierra Mágina', 'Sierra Sur']
...
def clasificar_elementos(lista_elementos):
    ...
    for elemento in lista_elementos:
        ...
        # if not categorias_asignadas["ciudad"] and any(ciudad.lower() in elemento_lower for ciudad in ciudades_jaen):
        #     ciudad_match = next(ciudad for ciudad in ciudades_jaen if ciudad.lower() in elemento_lower)
...
def procesar_inmuebles(base_url, tipo_transaccion, max_repetidos=10):
    """
    ...
    nuevos_guardados = 0  # Contador de inmuebles nuevos guardados
        ...
        if r.status_code == 200:
            ...
            for article in articles:
                ...
                if contacto:
                    ...
                    resultado = insertar_inmueble(inmueble_data)
                    if resultado:
                        nuevos_guardados += 1
...
# =============================================
# Diccionario de provincias Idealista
# =============================================
provincias = {
    ...
    "Burgos": "burgos", "Cáceres": "caceres", "Cádiz": "cadiz", "Cantabria": "cantabria",
    "Castellón": "castellon", "Ciudad Real": "ciudad-real", "Córdoba": "cordoba", "La Coruña": "la-coruna",
    "Cuenca": "cuenca", "Gerona": "girona", "Granada": "granada", "Guadalajara": "guadalajara",
    "Guipúzcoa": "guipuzcoa", "Huelva": "huelva", "Huesca": "huesca", "Islas Baleares": "illes-balears",
    "Jaén": "jaen", "León": "leon", "Lérida": "lleida", "Lugo": "lugo", "Madrid": "madrid",
    "Málaga": "malaga", "Murcia": "murcia", "Navarra": "navarra", "Orense": "ourense",
    "Palencia": "palencia", "Las Palmas": "las-palmas", "Pontevedra": "pontevedra", "La Rioja": "la-rioja",
    "Salamanca": "salamanca", "Segovia": "segovia", "Sevilla": "sevilla", "Soria": "soria",
    "Tarragona": "tarragona", "Santa Cruz de Tenerife": "santa-cruz-de-tenerife", "Teruel": "teruel",
    "Toledo": "toledo", "Valencia": "valencia", "Valladolid": "valladolid", "Vizcaya": "bizkaia",
    "Zamora": "zamora", "Zaragoza": "zaragoza", "Ceuta": "ceuta", "Melilla": "melilla"
}

# =============================================
# Leer provincias ya procesadas
# =============================================
archivo_progreso = "provincias_procesadas.txt"
provincias_hechas = set()

if os.path.exists(archivo_progreso):
    with open(archivo_progreso, "r", encoding="utf-8") as f:
        provincias_hechas = set(line.strip() for line in f.readlines())

# =============================================
# Bucle con reanudación automática
# =============================================
for nombre_provincia, url_provincia in provincias.items():
    if nombre_provincia in provincias_hechas:
        print(f"✅ Provincia ya procesada: {nombre_provincia}, saltando...")
        continue

    print(f"\n🟡 Procesando provincia: {nombre_provincia} ({url_provincia})")
    try:
        total_guardados = 0  # Contador de inmuebles guardados en esta provincia
        # Venta
        total_guardados += procesar_inmuebles(
            base_url=f"https://www.idealista.com/venta-viviendas/{url_provincia}-provincia/pagina-{{}}.htm?ordenado-por=fecha-publicacion-desc",
        ...
        total_guardados += procesar_inmuebles(
            base_url=f"https://www.idealista.com/alquiler-viviendas/{url_provincia}-provincia/pagina-{{}}.htm?ordenado-por=fecha-publicacion-desc",
        ...
        total_guardados += procesar_inmuebles(
            base_url=f"https://www.idealista.com/alquiler-habitacion/{url_provincia}-provincia/pagina-{{}}.htm?ordenado-por=fecha-publicacion-desc",
        ...
        with open(archivo_progreso, "a", encoding="utf-8") as f:
            f.write(nombre_provincia + "\n")
        print(f"✅ Provincia completada: {nombre_provincia}({total_guardados} inmuebles añadidos)")

    except Exception as e:
        print(f"❌ Error en {nombre_provincia}: {str(e)}")
        continue
